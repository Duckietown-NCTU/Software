...
For more info on algorithm and parameters please refer to the google doc:
 https://drive.google.com/open?id=0B49dGT7ubfmSX1k5ZVN1dEU4M2M
    ...
    def generateVote(self,segment):
        ...
        if segment.color == segment.YELLOW: # right lane is yellow
            if(p1[0] > p2[0]): # right edge of yellow lane
                d_i = d_i - self.linewidth_white
            else: # left edge of yellow lane
                d_i = - d_i
                phi_i = -phi_i
            d_i = d_i - self.lanewidth/2

        elif segment.color == segment.WHITE: # left lane is white
            if (p2[0] > p1[0]): # left edge of white lane
